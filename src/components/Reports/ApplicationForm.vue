...
export default {
  ...
  computed: {
    ...
    textbeforeAPP () {
      ...
      return `Згідно з навчальним планом підготовки ОР «${this.isMasters ? 'Магістр' : 'Бакалавр'}» за спеціальністю ${this.specialty.code} «${this.specialty.name}» ${this.facultyTextForApplication} та графіку навчального процесу на 2020-2021 н.р.`
    },
    textafterAPP () {
      return `1.Направити студентів ${this.isMasters ? '(магістрів 2020 року вступу)' : ''} ${this.isMasters ? 'п\'ятого' : this.courseTextForApplication} курсу на ${this.practice.name.toLowerCase()} практику ${this.courseTextForApplication === 'четвертого' ? ', що виконують дипломну роботу' : ''} по кафедрі ${this.department.name.toLowerCase()}.`
    },
    specialty () {
  ...
  methods: {
    ...
    createApplication () {
      ...
      const tableFooter = doc.createTable(8, 2).setWidth(docx.WidthType.PERCENTAGE, '105%')

      tableFooter.getCell(0, 0)
        .addContent(new docx.Paragraph('Помічник проректора').style('myStyles'))
        .CellProperties.Borders.addTopBorder(docx.BorderStyle.SINGLE, 1, 'white')
        .addBottomBorder(docx.BorderStyle.SINGLE, 1, 'white')
        .addStartBorder(docx.BorderStyle.SINGLE, 1, 'white')
        .addEndBorder(docx.BorderStyle.SINGLE, 1, 'white')
